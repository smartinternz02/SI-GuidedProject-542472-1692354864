...
app = Flask(__name__)
app.secret_key = 'a'
conn = ibm_db.connect("DATABASE=bludb; HOSTNAME=9938aec0-8105-433e-8bf9-0fbb7e483086.c1ogj3sd0tgtu0lqde00.databases.appdomain.cloud; PORT=32459; UID=grd70813;PASSWORD=4XbBD9IAngmyz5U5; SECURITY=SSL;SSLServerCertificate = DigiCertGlobalRootCA.crt", "", "")
...
def loginentered():
    ...
    if request.method == "POST":
        ...
        sql = "SELECT * FROM SB_TABLE1 WHERE EMAIL=? AND PASSWORD=?"  # from db2 sql table
        ...
        if account: 
            ...
            sql = "SELECT ROLE FROM SB_TABLE1 where email = ?"
            stmt = ibm_db.prepare(conn, sql)
            ibm_db.bind_param(stmt, 1, email)
            ibm_db.execute(stmt)
...
def signup():
    ...
    if request.method == 'POST':
        ...
        print(password)
        sql = "SELECT* FROM sb_table1 WHERE email= ?"
        stmt = ibm_db.prepare(conn, sql)
        ibm_db.bind_param(stmt, 1, email)
        ibm_db.execute(stmt)
        account = ibm_db.fetch_assoc(stmt)
...
def facultymarks():
    data=[]
    sql = "SELECT USERNAME from SB_TABLE1 WHERE ROLE=1"
    stmt = ibm_db.prepare(conn, sql)
    ibm_db.execute(stmt)
